...
mod erc20 {
    ...
    mod tests {
        ...
        fn transfer_work() {
            ...
            assert_eq!(contract.transfer(AccountId::from([0x0; 32]), 200), Ok(()));
            assert_eq!(contract.balance_of(AccountId::from([0x0; 32])), 200);
        ...
        fn transfer_from_work() {
            ...
            assert_eq!(
                contract.transfer_from(AccountId::from([0x1; 32]), AccountId::from([0x0; 32]), 20),
                Ok(())
            );
            assert_eq!(contract.balance_of(AccountId::from([0x0; 32])), 20);
            assert_eq!(contract.balance_of(AccountId::from([0x1; 32])), 980);
        }
    }
}
